use ac_pcap_parser::messages::ParsedMessage;
use std::fs::File;
use std::io::Read;
...
fn load_test_messages() -> Vec<ParsedMessage> {
    ...
    let mut file = File::open(pcap_path).expect("Failed to open test PCAP file");
    let mut buffer = Vec::new();
    file.read_to_end(&mut buffer).expect("Failed to read PCAP file");

    let (_, messages) = ac_pcap_parser::parse_pcap_file(&buffer)
