...
mod tui {
    ...
    use common::PacketHeaderFlags;
    use common::packet_flags::format_packet_flags;
    use common::tree::{TreeNode, collect_all_expandable_paths};
    use crossterm::{
        event::{
    ...
    fn load_packets(path: &Path) -> Result<Vec<PacketInfo>> {
        ...
        let (_packets, messages, _weenie_db) = parser.parse_pcap(file)?;
        ...
        for msg in messages {
            let info = PacketInfo {
                ...
                timestamp: "".to_string(), // Not available in ParsedMessage
                flags: "".to_string(),     // Not available in ParsedMessage
                packet_type: msg.message_type.clone(),
                size: serde_json::to_string(&msg.data).unwrap_or_default().len(),
                opcode: msg.opcode.clone(),
                sequence: 0, // Not available in ParsedMessage
...
#[derive(Parser)]
#[command(name = "tui")]
#[command(about = "Enhanced TUI for browsing .pcap network capture files", long_about = None)]
struct Cli {
    /// Path to .pcap file
    path: PathBuf,
}
