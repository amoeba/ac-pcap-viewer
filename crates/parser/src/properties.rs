...
pub fn read_spell_book(reader: &mut BinaryReader) -> Result<Vec<LayeredSpellId>> {
    ...
    let count = reader
        .read_u32()
        .context(format!("spell count at pos {}", reader.position()))? as usize;
    if count > 1000 {
        anyhow::bail!(
            "Suspicious spell count: {} at pos {}",
            count,
            reader.position()
        );
    }
    ...
    for i in 0..count {
        ...
        let id =
            reader
                .read_u16()
                .context(format!("spell {} id at pos {}", i, reader.position()))?
                as u32;
        let layer =
            reader
                .read_u16()
                .context(format!("spell {} layer at pos {}", i, reader.position()))?;
